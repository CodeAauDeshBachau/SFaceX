...
class StartScreen(QWidget):
    ...
    prediction_complete = Signal(str, bool)

    def __init__(self, behavior_model):
        super().__init__()
        self.behavior_model = behavior_model
        self.predicted_method = "FACE"  # Default prediction
        self.prediction_complete.connect(self.update_ui_after_prediction)
    ...
    def run_prediction(self):
        ...
        print("DEBUG: Starting prediction thread...")
        threading.Thread(target=self._get_prediction, daemon=True).start()

    def _get_prediction(self):
        try:
            recommendation = self.behavior_model.get_recommendation()
            method = recommendation["recommended_method"].upper()
            self.predicted_method = method
            self.prediction_complete.emit(method, False)
        except Exception as e:
            print(f"Prediction error: {e}")
            self.predicted_method = "FACE"
            self.prediction_complete.emit("FACE", True)

    def update_ui_after_prediction(self, method, error):
        if error:
            self.status_label.setText("Could not predict. Defaulting to Face Unlock.")
        else:
            self.status_label.setText(
                f"Prediction: You will likely use <b>{method}</b>"
            )
        ...
        try:
            ...
            if result == "enrolled_person":
                ...
                try:
                    ...
                    filename = f"./positive/face_{timestamp}.png"
                    cv2.imwrite(filename, face_roi)
                    print(f"DEBUG: Saved authenticated face to {filename}")
                except Exception as save_err:
                    print(f"DEBUG: Failed to save face image: {save_err}")
    ...
    def prompt_for_enrollment(self):
        ...
        if (
            ...
            if msg_box.clickedButton() == yes_button:
                ...
                self.authenticator.calculate_roc_and_find_threshold_template(
                    negative_sample_paths=None, positive_sample_paths=None
                )
                print("DEBUG: Enrollment complete.")
